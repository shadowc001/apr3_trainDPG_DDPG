...
This script trains two agents on the MountainCarContinuous-v0 environment concurrently:
...
class DPGAgentNoNN:
    def __init__(self, state_dim, action_dim, max_action, actor_lr=1e-2, critic_lr=1e-2, gamma=0.95, reward_scale=0.0):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.max_action = max_action
        ...
        self.gamma = gamma
        self.reward_scale = reward_scale  # Tried adding a scaling factor for rewards, this did not help

    def select_action(self, state):
        return self.actor.predict(state)

    def train_step(self, state, action, reward, next_state, done):
        # Scale reward to amplify the signal if necessary.
        ...
        # Compute gradient from critic with respect to the action.
        # For a linear critic Q(s,a)=w_s^T s + w_a^T a + b, the derivative w.r.t. a is simply w_a.
        grad_from_critic = self.critic.weights[self.state_dim:self.state_dim + self.action_dim]
        # Update actor using the gradient from the critic.
...
def train_agent(agent, env, num_episodes=200, batch_size=64, agent_name="agent",
    ...
    log_filename = os.path.join(log_dir, f"{agent_name}_log_{run_date}.csv")
    if log_lock:
        with log_lock:
...
def train_ddpg_agent(num_episodes=200, batch_size=64, run_date=None, log_lock=None):
    # Use MountainCarContinuous-v0 (continuous action space)
    env = gym.make("MountainCarContinuous-v0")
    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]
    max_action = float(env.action_space.high[0])
    ddpg_agent = DDPGAgent(state_dim, action_dim, max_action, use_replay=True, use_target=True, noise_std=0.2)
...
def train_dpg_agent(num_episodes=200, run_date=None, log_lock=None):
    ...
    action_dim = env.action_space.shape[0]
    max_action = float(env.action_space.high[0])
    dpg_agent = DPGAgentNoNN(state_dim, action_dim, max_action, actor_lr=1e-3, critic_lr=2e-3, gamma=0.95)
